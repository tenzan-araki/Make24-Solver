...
So the code here should be useful for those who have a smart cousin. For the sole purpose of winning, just try running the code in the notebook `Demo of Make 24 Solver.ipynb`. Solutions are found in milliseconds, so it is limited by the user's typing speed. I have no plans to improve this by enabling audio or camera functionalities, because it should already be sufficiently fast to defeat most cousins. It would be a perfect way to procrastinate though.
