...
def make24_analysis_1():
    ...
    M = 13
    # numbers in a deck of cards
    count = 0
    # number of solutions initialized to 0
    ...
    for A1 in range(1, M + 1):
        for A2 in range(1, A1 + 1):
            for A3 in range(1, A2 + 1):
                for A4 in range(1, A3 + 1):
                    count.append(
                        make24_solver(A1, A2, A3, A4, mode="all", print_ok=False)
                    )

    n, bins, patches = plt.hist(
        x=count, bins="auto", color="#0504aa", alpha=0.7, rwidth=0.85
    )
